...
for i in range(35):
	...
	for j in range(2805): #nakon svake epohe prolazimo kroz skup za validaciju
		lval+=loss.eval(inpt,session=sess)
	...
	print('----------------------------------------------------------')
	print('|' + "{:^10}".format('Tacnost') + '|' + "{:^22,.3f}".format(acc/16834) + '|' + "{:^22,.3f}".format(acc_val/5610)+'|')
	print('----------------------------------------------------------')
	print("\n")
...
ltest=0
acctest=0
#rezultati=['zero','one','two','three','four','five','six','seven','eight','nine','ten','eleven','twelve','thirteen','fourteen','fifteen','sixteen','draw']
...
for i in range(2805):
    ...
    #f.write(rezultati[int(tf.argmax(y,1))] + " " + rezultati[int(tf.argmax(y_est,1))] + '\n')
